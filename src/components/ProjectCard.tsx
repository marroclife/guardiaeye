    ...
    >
      ...
      <div className="flex items-start justify-between mb-1.5 md:mb-2">
        ...
        {project.contract_number && (
          <span className={cn(
            "px-2 py-0.5 rounded text-xs font-mono flex-shrink-0 bg-marroc-dourado/10 text-marroc-dourado border border-marroc-dourado/20",
      ...
      {project.lead?.phone && (
        <div className={cn(
          ...
          <Phone className={cn("text-marroc-esmeralda", compact ? "w-3 h-3" : "w-3 h-3")} />
      ...
      >
        <div className="flex items-center gap-1.5 text-marroc-salvia/70">
          <Calendar className={cn("text-marroc-salvia", compact ? "w-3 h-3" : "w-3 h-3")} />
      ...
      <div className={cn(
        ...
        <div className={cn(
          "flex items-center gap-1.5 font-light",
          compact ? "text-[10px]" : "text-xs",
          ...
          {overdue ? (
            <>
              ...
              <span>Atrasado {Math.abs(daysUntil || 0)}d</span>
            </>
          ) : closeDeadline ? (
            <>
              <AlertCircle className={cn("", compact ? "w-3 h-3" : "w-3 h-3")} />
              <span>Faltam {daysUntil}d</span>
            </>
          ) : (
            <>
              <ClipboardList className={cn("", compact ? "w-3 h-3" : "w-3 h-3")} />
              <span>{project.features.length} funcionalidades</span>
            </>
          )}
        </div>
        {project.status === 'concluido' && (
          <CheckCircle2 className={cn("text-marroc-esmeralda", compact ? "w-3 h-3" : "w-4 h-4")} />
        )}
      </div>
    </div>
  );
