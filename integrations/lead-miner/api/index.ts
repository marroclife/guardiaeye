...
app.post('/api/leads/search', async (req, res) => {
  try {
    ...
    if (filterMissingWebsiteOnly) {
      filteredPlaces = rawPlaces.filter(p => !p.website || /booking\.com|airbnb\.com|expedia\.com/i.test(p.website));
    }
...
app.post('/api/leads/:id/pitch', async (req, res) => {
  try {
    ...
    res.json({ success: true, outreach: diag.outreach });
  } catch (err: any) {
    res.status(500).json({ success: false, error: err.message });
  }
});
...
app.post('/api/leads/export-to-nexus', async (req, res) => {
  try {
    ...
    if (!Array.isArray(leadIds) || leadIds.length === 0) {
      return res.status(400).json({ success: false, error: 'Forneça um array de leadIds para exportar.' });
    }
    const leadsToExport = leadIds.map((id) => leadDb.getById(id)).filter(Boolean);
    if (leadsToExport.length === 0) {
      return res.status(404).json({ success: false, error: 'Nenhum lead encontrado com os IDs fornecidos.' });
    }
    const result = await exportLeadsToNexus(leadsToExport);
    if (!result.success) {
      return res.status(500).json({ success: false, error: result.errors.join('; '), exported: result.exported, skipped: result.skipped });
    }
    res.json({
      success: true,
...
// Static files in production (Vercel serves /dist)
